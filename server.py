...
class TimelineHandler(SimpleHTTPRequestHandler):
    ...
    def handle_rename_asset(self) -> None:
        try:
            ...
            match = next((image for image in images if image.get("filename") == old_filename), None)
            if match is None:
    ...
    def handle_delete_asset(self) -> None:
        try:
            ...
            assets_payload = json.loads(ASSETS_DATA_PATH.read_text(encoding="utf-8"))
            images = assets_payload.get("images", [])
            next_images = [image for image in images if image.get("filename") != filename]
    ...
    def _assert_has_keys(self, obj: dict[str, Any], keys: list[str], label: str) -> None:
        missing = [key for key in keys if key not in obj]
        if missing:
            raise ValueError(f"{label} 缺少字段: {', '.join(missing)}")

    def _validate_graph_refine_request(self, payload: dict[str, Any]) -> None:
        self._assert_has_keys(payload, ["protocol", "schema_version", "session", "window", "graph_snapshot", "candidate_nodes", "instructions"], "request")
        protocol = payload["protocol"]
        if not isinstance(protocol, dict):
            raise ValueError("protocol 必须是 object")
        self._assert_has_keys(protocol, ["schema_version", "schema_path"], "protocol")
        if protocol["schema_version"] != MODEL_SCHEMA_VERSION or payload["schema_version"] != MODEL_SCHEMA_VERSION:
        ...
        self._assert_has_keys(graph_snapshot, ["node_count", "edge_count", "nodes", "edges"], "graph_snapshot")
        self._assert_has_keys(instructions, ["goal", "allowed_operations"], "instructions")
        ...
        if not isinstance(graph_snapshot["nodes"], list) or not isinstance(graph_snapshot["edges"], list):
            raise ValueError("graph_snapshot.nodes / edges 必须是数组")

    def _validate_graph_refine_response(self, response: dict[str, Any]) -> None:
        self._assert_has_keys(response, ["schema_version", "request_id", "summary", "operations"], "response")
        if response["schema_version"] != MODEL_SCHEMA_VERSION:
            raise ValueError(f"response.schema_version 必须是 {MODEL_SCHEMA_VERSION}")
        summary = response["summary"]
        self._assert_has_keys(summary, ["reasoning_mode", "operation_count"], "response.summary")
    ...
    def _prune_operations(
        ...
        for op in operations:
            ...
            if op["op"] == "rename_node":
                ...
                old_label = target.get("value", "").strip() if target.get("by") == "label" else ""
            ...
            if op["op"] == "mark_revisited":
                ...
                if target.get("by") == "label" and target_value and target_value not in raw_text:
                    continue
        ...
        id_to_label = {node_id: label for label, node_id in existing_labels.items() if node_id}
        focus_index = next((index for index, op in enumerate(pruned) if op["op"] == "set_focus"), None)
        if focus_index is not None:
            ...
            if focus_label in weak_focus_labels:
                replacement_label = next((label for label in preferred_focus_labels if label in existing_labels), None)
    ...
    def _coerce_node_ref(self, value: Any) -> dict[str, str] | None:
        if isinstance(value, dict):
            if value.get("by") in {"id", "label"} and isinstance(value.get("value"), str):
    ...
    def _coerce_operation_shape(self, operation: dict[str, Any]) -> dict[str, Any]:
        ...
        if op_type == "merge_node":
            from_ref = self._coerce_node_ref(operation.get("from") or operation.get("source"))
            to_ref = self._coerce_node_ref(operation.get("to") or operation.get("target"))
            if from_ref and to_ref:
                return {"op": op_type, "from": from_ref, "to": to_ref}
            return {}

        if op_type == "set_node_type":
            target = self._coerce_node_ref(operation.get("target") or operation.get("node"))
        ...
        if op_type == "add_pending_node":
            ...
            connect_to = self._coerce_node_ref(operation.get("connect_to") or operation.get("target") or operation.get("parent"))
            if isinstance(node, dict) and isinstance(node.get("label"), str) and isinstance(node.get("description"), str):
                return {
                    "op": op_type,
                    "node": {"label": node["label"], "description": node["description"]},
        ...
        if op_type == "update_node_description":
            target = self._coerce_node_ref(operation.get("target") or operation.get("node"))
            description = operation.get("description") or operation.get("new_description")
        ...
        if op_type in {"add_edge", "remove_edge"}:
            source = self._coerce_node_ref(operation.get("source") or operation.get("from"))
            target = self._coerce_node_ref(operation.get("target") or operation.get("to") or operation.get("connect_to"))
            if source and target:
                return {"op": op_type, "source": source, "target": target}
            return {}

        if op_type in {"mark_revisited", "set_focus"}:
            target = self._coerce_node_ref(operation.get("target") or operation.get("node") or operation.get("focus"))
            if target:
    ...
    def _gemini_think_different_response(self, *, topic: str, tone: str) -> dict[str, Any]:
        if not GEMINI_API_KEY:
            raise ValueError("GEMINI_API_KEY 未配置")
        ...
        for item in items[:6]:
            ...
            normalized_items.append(
                {
                    "perspective": str(item.get("perspective", "")).strip() or "不同视角",
    ...
    def _mock_graph_refine_response(
        ...
        *,
        ...
        candidate_labels = [node.get("label", "") for node in payload["candidate_nodes"]]
